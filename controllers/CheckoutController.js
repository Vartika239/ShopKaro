...
async function getUserRecord(req, res) {
  try {
    let finalData = await Checkout.find({user:req.params.userid})
      .sort({ _id: -1 })
      .populate("user", ["name","username","email","phone","address","pin","city","state"])
      .populate({
        path:"products.product",
...
async function getRecord(req, res) {
  try {
    let finalData = await Checkout.find()
      .sort({ _id: -1 })
      .populate("user", ["name","username","email","phone","address","pin","city","state"])
      .populate({
        path:"products.product",
...
async function getSingleRecord(req, res) {
  try {
    let finalData= await Checkout.findOne({ _id: req.params._id })
    .populate("user", ["name","username","email","phone","address","pin","city","state"])
    .populate({
      path:"products.product",
      select:"name brand color size basePrice stockQuantity pic",
...
console.log('Checkout Found:', data); // Log the data fetched from the database
